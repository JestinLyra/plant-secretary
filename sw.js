const CACHE='plant-secretary-v3';
const ASSETS=['./','./index.html','./manifest.webmanifest','./icon.svg','./home-art.css','./home-art.js'];
const decorate=async response=>{
  const text=await response.text();
  const html=text.replace('</head>','<link rel="stylesheet" href="home-art.css?v=3"></head>').replace('</body>','<script src="home-art.js?v=3"></script></body>');
