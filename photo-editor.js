...
function applyWithin(root){if(!root)return;if(root.matches?.('[id^="photo-"] img,[id^="hero-"] img')){const box=root.parentElement;const id=box.id.replace(/^(photo|hero)-/,'');applyTo(root,id)}root.querySelectorAll?.('[id^="photo-"] img,[id^="hero-"] img').forEach(img=>{const box=img.parentElement;const id=box.id.replace(/^(photo|hero)-/,'');applyTo(img,id)})}
function applyAll(){applyWithin(document)}
async function getPhotoFor(id){try{return await getPhoto(id)}catch(e){return null}}
async function deleteStoredPhoto(id){try{if(typeof db!=='function')throw new Error('Photo database unavailable');const d=await db();await new Promise((resolve,reject)=>{const tx=d.transaction('photos','readwrite');tx.objectStore('photos').delete(id);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)});d.close();clearView(id);return true}catch(err){console.warn('Delete photo failed',err);return false}}
async function syncDeleteButtons(root=document){const buttons=[...root.querySelectorAll?.('[data-delete-photo]')||[]];await Promise.all(buttons.map(async b=>{const has=!!(await getPhotoFor(b.dataset.deletePhoto));b.hidden=!has}))}
function refreshPhotoUI(id){if(typeof renderCollection==='function')renderCollection();if($('#plantModal')?.classList.contains('open')&&typeof openModal==='function')openModal(id);setTimeout(()=>{applyAll();syncDeleteButtons()},80)}
async function deletePhoto(id){const p=(window.plants||[]).find(x=>String(x.id)===String(id));if(!confirm(`Delete the uploaded photo${p?.name?` for ${p.name}`:''}?`))return;const ok=await deleteStoredPhoto(id);if(!ok){if(typeof toast==='function')toast('Photo could not be deleted');return}closeManager();refreshPhotoUI(id);if(typeof toast==='function')toast('Photo deleted')}
...
async function managePhoto(id){const blob=await getPhotoFor(id);if(!blob){choosePhoto(id);return}manageId=id;const m=ensureManager();const deleteButton=m.querySelector('#photoDelete');if(deleteButton)deleteButton.dataset.deletePhoto=String(id);m.classList.add('open')}
...
document.addEventListener('click',e=>{const target=e.target instanceof Element?e.target:null;if(!target)return;const del=target.closest('[data-delete-photo]');if(del){e.preventDefault();e.stopPropagation();deletePhoto(del.dataset.deletePhoto);return}const box=target.closest('.hero-photo');if(!box)return;const id=box.id?.replace(/^hero-/,'');if(!id)return;e.preventDefault();e.stopPropagation();managePhoto(id)},true);
