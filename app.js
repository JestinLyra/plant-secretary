...
function renderProfile(id,page=1){
 ...
 let body='';
 if(page===1) body=`<article class="profile-poster"><div class="quick-care-row"><div><b>${sunlight[p.id]||p.sun||'🌤️'}</b><span>${g.position.split('.')[0]}</span></div><div><b>💧</b><span>${g.moisture}</span></div><div><b>pH</b><span>${phFor(p,g)}</span></div></div><section class="poster-grid poster-grid-natural"><div class="poster-column">${careCard('☀️','POSITION',g.position)}${careCard('💧','WATERING',g.water)}</div><div class="poster-column">${careCard('🪴','SOIL',g.soil)}${careCard('🌿','FEEDING',`<strong>${g.feed}</strong><br>${g.feedNote}`)}</div></section><article class="grow-tip"><b>💡 GROW TIP</b><p>${g.grow}</p></article></article>`;
