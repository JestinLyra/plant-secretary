...
function renderProfile(id,page=1){
 ...
 let body='';
 if(page===1) body=`<article class="profile-poster quick-guide-v110"><section class="quick-info-grid">${quickInfoCard('light','LIGHT',g.position)}${quickInfoCard('watering','WATERING',`${g.moisture}. ${g.water}`)}${quickInfoCard('soil','SOIL',g.soil)}<article class="quick-info-card quick-info-feeding quick-feeding-card"><div class="quick-info-icon">${quickGuideIcon('feeding')}</div><div class="quick-info-copy"><span class="quick-info-label">FEEDING</span><p><strong>${g.feed}</strong></p><p>${g.feedNote}</p></div></article></section>${quickFullSection('ph','pH',`<p>${phFor(p,g)}</p>`,'quick-ph-card')}${quickFullSection('maintenance','HANDS-ON CARE',`<ul>${(g.maint||[]).map(x=>`<li>${x}</li>`).join('')}</ul>`,'quick-maintenance-card')}${quickFullSection('tip','GROW TIP',`<p>${g.grow}</p>`,'quick-grow-card')}</article>`;
 ...
 el('profileContent').innerHTML=`${tabs}${identity}<div class="profile-page">${body}</div><button id="profileWaterBtn" class="profile-water-btn" type="button">💧 ${watered}</button>`;
