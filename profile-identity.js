...
function editFields(id){const p=plantsList().find(x=>String(x.id)===String(id));if(!p)return;closeEditMenu();const common=prompt('Common name(s)',p.common||'');if(common===null)return;const botanical=prompt('Botanical name',p.botanical||'');if(botanical===null)return;const botanicalName=botanical.trim();if(!botanicalName){notify('Botanical name cannot be empty');return;}p.common=common.trim();p.botanical=botanicalName;delete p.habit;if(typeof save==='function')save();if(typeof renderAll==='function')renderAll();if(typeof window.openModal==='function')window.openModal(id)}
...
function openEditMenu(id){
  ...
  const editBtn=document.createElement('button');editBtn.type='button';editBtn.className='plant-edit-action';editBtn.textContent='Edit common & botanical names';editBtn.addEventListener('click',()=>editFields(id));
