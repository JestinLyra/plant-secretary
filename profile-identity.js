...
function editFields(id){const p=plantsList().find(x=>String(x.id)===String(id));if(!p)return;closeEditMenu();const name=prompt('Plant / display name',p.name||'');if(name===null)return;const botanical=prompt('Botanical name',p.botanical||'');if(botanical===null)return;const currentLocation=String(p.location||'').toLowerCase()==='outdoor'?'Outdoor':'Indoor';const locationInput=prompt('Location — enter Indoor or Outdoor',currentLocation);if(locationInput===null)return;const normalizedLocation=String(locationInput).trim().toLowerCase();if(normalizedLocation!=='indoor'&&normalizedLocation!=='outdoor'){notify('Location must be Indoor or Outdoor');return;}p.name=name.trim()||p.name;p.botanical=botanical.trim();p.location=normalizedLocation==='outdoor'?'Outdoor':'Indoor';delete p.common;delete p.habit;if(typeof save==='function')save();if(typeof renderAll==='function')renderAll();if(typeof window.openModal==='function')window.openModal(id)}
